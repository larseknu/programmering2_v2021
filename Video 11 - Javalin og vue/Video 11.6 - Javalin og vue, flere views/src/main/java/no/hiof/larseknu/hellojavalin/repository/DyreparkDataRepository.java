...
public class DyreparkDataRepository implements DyreparkRepository {
    ...
    @Override
    public Dyr getDyr(String dyreParkNavn, int dyreId) {
        DyrePark dyreParken = getDyrepark(dyreParkNavn);

        for (Dyr dyret : dyreParken.getDyrIDyreParken()) {
            if (dyret.getID() == dyreId)
                return dyret;
        }
